...
,('Pasztet z wątróbki', 'Składniki: 500g wątróbki drobiowej, 1 cebula, 2 ząbki czosnku, 1 jajko, 1/2 szklanki bułki tartej, 1 łyżeczka soli, pieprz do smaku',
'1. Wątróbkę ugotuj lub usmaż do miękkości. 2. Posiekaj cebulę i czosnek, podsmaż je na patelni. 3. W blenderze zmiksuj wątróbkę, cebulę, czosnek, jajko oraz przyprawy. 4. Dodaj bułkę tartą i wymieszaj. 5. Przełóż do formy, piecz przez 45 minut w temperaturze 180°C.')
...
(1, (SELECT id FROM ingredients WHERE name = 'Makaron spaghetti')),
...
-- Pomidorowa zupa
(2, (SELECT id FROM ingredients WHERE name = 'Pomidory')),
...
(2, (SELECT id FROM ingredients WHERE name = 'Sól')),
(2, (SELECT id FROM ingredients WHERE name = 'Pieprz')),
-- Kurczak pieczony z ziemniakami
(3, (SELECT id FROM ingredients WHERE name = 'Ziemniaki')),
(3, (SELECT id FROM ingredients WHERE name = 'Papryka')),
(3, (SELECT id FROM ingredients WHERE name = 'Rozmaryn')),
(3, (SELECT id FROM ingredients WHERE name = 'Sól')),
(3, (SELECT id FROM ingredients WHERE name = 'Pieprz')),
-- Dalsze przepisy...
;
